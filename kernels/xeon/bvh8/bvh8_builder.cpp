...
namespace embree
{
  namespace isa
  {
    ...
    {
      /* allocate leaf node */
      cout << "BVH8 Create Leaf Called" << endl; 
    ...
    {
      cout << "BVH8 Create Large Leaf Called" << endl;
#if defined(_DEBUG)
    ...
    {
      cout << "BVH8 createNode called" << endl;
	...
	*record.dst = parent->createLargeLeaf(threadIndex,record.prims,record.pinfo,record.depth+1); return 0;
    ...
    {
	cout << "BVH8 finish_build called" << endl;						
    ...
    {
	cout << "BVH8 continue_build called" << endl;
      /* finish small tasks */
    ...
    {
	cout << "BVH8 build_parallel called" << endl;
      while (activeBuildRecords)
      {
	cout << "while activeBuildRecords" << endl;
	taskMutex.lock();
	if (tasks.size() == 0) {
	  taskMutex.unlock();
	  cout << "continue " << endl;
    ...
    {
	cout << "BVH8 layout_top_nodes called" << endl;
      if (node.isBarrier()) {
	cout << "node is barrier" << endl;
	node.clearBarrier();
	return node;
      }
      else if (!node.isLeaf()) 
	...
	BVH8Rotate::rotate(bvh,bvh->root);
      ...
      cout << "pinfo.size() " << pinfo.size() << endl;
