...
Vec3fa renderPixelStandard(float x, float y, const Vec3fa& vx, const Vec3fa& vy, const Vec3fa& vz, const Vec3fa& p)
{
  ...
  RTCRay ray;
  ray.org = p;
  ray.dir = normalize(x*vx + y*vy + vz); // vx, vy vector, x,y coord
  ray.tnear = 0.0f;
  ray.tfar = inf;
...
extern "C" void device_render (int* pixels,
                    ...
                    const Vec3fa& p)
{
  const int numTilesX = (width +TILE_SIZE_X-1)/TILE_SIZE_X;//(512+7)/8
  //numTilesX = 64
  const int numTilesY = (height+TILE_SIZE_Y-1)/TILE_SIZE_Y;//(512+7)/8
  //numTilesY = 64
